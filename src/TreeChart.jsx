...
const TreeChart = ({ data }) => {
  ...
  const renderBar = (data) => {
    ...
    const dx = 20;
    const dy = innerWidth / 8
    const tree = d3.tree().nodeSize([dx, dy])

    root.y0 = innerHeight / 2;
    root.x0 = tree_margin.left;
    ...
    const gNode = g.append("g")
      ...
      .attr("transform",`translate(${tree_margin.left+40}, ${innerWidth/4})`);

    const gLink = g.append("g")
      .attr("fill", "none")
      .attr("stroke", "#555")
      .attr("stroke-opacity", 0.4) // change later
      .attr("transform",`translate(${tree_margin.left+40}, ${innerWidth/4})`)
    ...
    function update(event, source) {
      ...
      var variable_height = leaves.length * 20 + margin.top + margin.bottom + tree_margin.top + tree_margin.bottom;
      
      select(svg.current).transition().attr("height",variable_height);
      ...
      nodeEnter.append("text")
        ...
        .text(d => d._children ? d.data.variable_description : "")
        .clone(true).lower()
